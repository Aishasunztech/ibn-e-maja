...
import com.sunztech.ibnemaja.Models.HadithItem;
...
public class DatabaseAccess {
    ...
    public ArrayList<HadithItem> getChapterHadith(int chapterId) {
        ...
        c = db.rawQuery("select * from tbl_IbneMaja WHERE Kitab_ID = " + chapterId, new String[]{});
    ...
    public void updateBookMark(HadithItem hadithItem)
    {
        ...
        db.update("tbl_IbneMaja", args, "hadees_number = " + hadithItem.getHadees_number(), null);
    }

    public ArrayList<HadithItem> getBookMarks()
    {
        HadithItem hadees = null;
        c = db.rawQuery("select * from tbl_IbneMaja WHERE is_bookmarked = " + 1, new String[]{});
        ArrayList<HadithItem> hadeesItems = new ArrayList<>();
        if (c != null) {
            while (c.moveToNext()) {
    ...
    public ArrayList<HadithItem> searchHadith(String columnName, String searchingValue) {
        ...
        if(columnName.equals("hadees_number")) {
            c = db.rawQuery("select * from tbl_IbneMaja WHERE " + columnName + " = " + searchingValue, new String[]{});

        }
        else{
            c = db.rawQuery("select * from tbl_IbneMaja WHERE "  + columnName + " like '%" + searchingValue + "%'", new String[]{});
